...
#Removiendo filas con SalePrice Faltantes
Data_Train <- Data_Train[!is.na(Data_Train$SalePrice), ]
...
library(caret)
set.seed(1000)
RFHousePrices = randomForest(SalePrice~ , data=Data_Train)
summary(RFHousePrices)




subset_RFHousePrices = subset(Data_Test,select=c(MSSubClass,LotArea,OverallQual,OverallCond,YearBuilt,MasVnrArea,BsmtUnfSF,TotalBsmtSF,X1stFlrSF,X2ndFlrSF,BsmtFullBath,BedroomAbvGr,KitchenAbvGr,TotRmsAbvGrd,GarageCars,WoodDeckSF,ScreenPorch))
...
MySubmission = data.frame(Id = Data_Test$Id, SalePrice = RFPredTest)
